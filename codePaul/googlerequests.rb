...
def yahooCount(s)
  ...
  s = CGI.escape(s) #s.gsub!(" ", "+")
  q = 'http://search.yahoo.com/search?p='+s
                    #q = p + CGI.escape(q)
  ...
  i = page_content.index('id="resultCount">')
...
def bingCount(s)
  ...
  s = CGI.escape(s) #s.gsub!(" ", "+")
  q = 'http://www.bing.com/search?q='+s

  page_content = open(q)
                    #puts q
                    #puts page_content
  return 0 if page_content.index("Keine Ergebnisse") != nil
